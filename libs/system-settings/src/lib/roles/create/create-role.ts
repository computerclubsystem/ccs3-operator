...
import { MessageTransportService, NotificationType, ValidatorsService, IconName } from '@ccs3-operator/shared';
import { NotificationService } from '@ccs3-operator/notification';
import { LinkedListsComponent } from '@ccs3-operator/linked-lists';
import { FormControls } from './internals';
...
export class CreateRoleComponent implements OnInit {
  ...
  onSave(): void {
    ...
    if (isCreate) {
      ...
      if (currentRole) {
        ...
        this.notificationSvc.show(NotificationType.error, translate(`The role was not loaded`), null, IconName.error, null);
        return;
      }
    }
  }
  ...
  processUpdateRoleWithPermissionsReplyMessage(updateRoleWithPermissionsReplyMsg: UpdateRoleWithPermissionsReplyMessage): void {
    ...
    this.notificationSvc.show(NotificationType.success, translate(`The role was updated`), null, IconName.check, updateRoleWithPermissionsReplyMsg);
  }
  ...
  processCreateRoleWithPermissionsReplyMessage(createRoleWithPermissionsReplyMsg: CreateRoleWithPermissionsReplyMessage): void {
    ...
    this.notificationSvc.show(NotificationType.success, translate(`The role was created`), null, IconName.check, createRoleWithPermissionsReplyMsg);
  }
