...
import { DurationFormControls, FormControls, FromToFormControls, Signals } from './internals';
...
export class CreateTariffComponent implements OnInit {
  ...
  processUpdateTariffReplyMessage(updateTariffReplyMsg: UpdateTariffReplyMessage): void {
    if (!updateTariffReplyMsg.header.failure) {
      this.notificationSvc.show(NotificationType.success, translate('Tariff updated'), null, IconName.check, updateTariffReplyMsg);
    }
  }

  processCreateTariffReplyMessage(createTariffReplyMsg: CreateTariffReplyMessage): void {
    if (!createTariffReplyMsg.header.failure) {
      this.notificationSvc.show(NotificationType.success, translate('Tariff created'), null, IconName.check, createTariffReplyMsg);
    }
  }
  ...
  processRestrictDurationStartChanges(restrictStartValue: boolean | null): void {
    ...
    if (this.form.value.type) {
      this.createTariffSvc.modifyDurationGroupValidators(this.form.controls.durationTypeGroup.controls, this.form.value.type.id as TariffType);
    }
  }
  ...
  processTariffTypeItemChanges(tariffTypeItem: NumericIdWithName | null): void {
    ...
    this.createTariffSvc.modifyDurationGroupValidators(this.form.controls.durationTypeGroup.controls, tariffType as TariffType);
    this.createTariffSvc.modifyFromToGroupValidators(this.form.controls.fromToTypeGroup.controls, tariffType as TariffType);
  }
