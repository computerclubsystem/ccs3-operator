...
import {
  HashService, MessageTransportService, NotificationType, SorterService, TimeConverterService,
  ValidatorsService, IconName
...
import { DurationFormControls, FormControls, Signals } from './internals';
...
@Component({
  ...
  templateUrl: 'create-prepaid-tariff.html',
  imports: [
...
export class CreatePrepaidTariffComponent implements OnInit {
  ...
  processCreatePrepaidTariffReplyMessage(createPrepaidTariffReplyMsg: CreatePrepaidTariffReplyMessage): void {
    if (!createPrepaidTariffReplyMsg.header.failure) {
      this.notificationSvc.show(NotificationType.success, translate('Tariff with ID (id) created', { id: createPrepaidTariffReplyMsg.body.tariff.id }), null, IconName.check, createPrepaidTariffReplyMsg);
