  ...
  sortByMany<TItem>(
    ...
    items.sort((left, right) => {
      for (const sortInfo of sortInfos) {
        ...
        const leftValueNullish = leftValue === null || leftValue === undefined;
        const rightValueNullish = rightValue === null || rightValue === undefined;
        if (leftValueNullish && rightValueNullish) {
          return 0;
        }
        if (leftValueNullish) {
          return 1 * sortSign;
        }
        if (rightValueNullish) {
          return -1 * sortSign;
        }
