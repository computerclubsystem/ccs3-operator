...
import {
  Group, groupArrayBy, MessageTransportService, NotificationType, NoYearDatePipe
...
@Component({
  ...
  templateUrl: 'signed-in-users.html',
...
export class SignedInUsersComponent implements OnInit {
  ...
  processForceSignOutAllUserSessionsReplyMessage(replyMsg: ForceSignOutAllUserSessionsReplyMessage, username: string): void {
    ...
    this.notificationSvc.show(NotificationType.success, title, description, IconName.check, replyMsg);
    this.loadSignedInUsers();
  }
