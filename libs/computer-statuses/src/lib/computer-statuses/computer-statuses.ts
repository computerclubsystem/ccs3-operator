...
import { ShiftStatusComponent } from '../shift-status/shift-status';
import { ShiftCompletedEventArgs } from '../shift-status/declarations';
import { RemainingTimeRankComponent } from '../remaining-time-rank/remaining-time-rank';
import { RechargePrepaidTariffComponent } from '../recharge-prepaid-tariff/recharge-prepaid-tariff';
...
@Component({
  ...
  templateUrl: 'computer-statuses.html',
  styleUrls: ['computer-statuses.css'],
  imports: [
...
export class ComputerStatusesComponent implements OnInit, AfterViewInit {
  ...
  executeShutdownStoppedGlobalBulkAction(): void {
    ...
    this.messageTransportSvc.sendAndAwaitForReply(reqMsg).pipe(
      ...
      this.notificationSvc.show(NotificationType.success, notificationMessage, null, IconName.check, replyMsg);
    });
  }
  ...
  executeSetNoteBulkAction(bulkActionData: BulkActionData): void {
    ...
    this.messageTransportSvc.sendAndAwaitForReply(reqMsg).pipe(
      ...
      this.notificationSvc.show(NotificationType.success, translate('Note has been set'), null, IconName.check, replyMsg);
    });
  }
  ...
  processCompleteShiftReplyMessage(replyMsg: CompleteShiftReplyMessage): void {
    ...
    this.notificationSvc.show(NotificationType.success, msg, null, IconName.check, replyMsg);
  }
  ...
  onTariffSelected(selectedTariff: Tariff, item: DeviceStatusItem): void {
    ...
    if (!canUseTariff.canUse) {
      this.notificationSvc.show(NotificationType.warn, translate(`The tariff '(tariffName)' can't be used right now`, { tariffName: selectedTariff.name }), `Current time is not in the tariff's From-To period`, IconName.priority_high, item);
    }
  }
  ...
  processDeleteDeviceContinuationReplyMessage(replyMsg: DeleteDeviceContinuationReplyMessage): void {
    if (replyMsg.header.failure) {
      ...
      this.notificationSvc.show(NotificationType.error, msg, null, IconName.priority_high, replyMsg);
      return;
    }
  ...
  processSetDeviceStatusNoteReplyMessage(replyMsg: SetDeviceStatusNoteReplyMessage): void {
    ...
    this.notificationSvc.show(NotificationType.success, translate('Computer note was set'), null, IconName.check, replyMsg);
  }
  ...
  onSetContinuation(item: DeviceStatusItem): void {
    if (!item.selectedContinueWithTariffId) {
      ...
      this.notificationSvc.show(NotificationType.warn, msg, null, IconName.priority_high, item);
      return;
    }
  ...
  processCreateDeviceContinuationReplyMessage(replyMsg: CreateDeviceContinuationReplyMessage, item: DeviceStatusItem): void {
    if (replyMsg.header.failure) {
      ...
      this.notificationSvc.show(NotificationType.warn, msg, null, IconName.priority_high, { replyMsg, deviceStatusItem: item });
      return;
    }
    const msg = translate(`Continuation saved`);
    this.notificationSvc.show(NotificationType.success, msg, null, IconName.check, { replyMsg, deviceStatusItem: item });
  }
